...
/** Saves styles to the catalog and confirms with a toast. Resolves to true when it worked. */
export function useAddToLibrary() {
  const bulkCreate = useBulkCreateStyles();
  const { toast } = useToast();

  const add = async (styles: StyleFormValues[]): Promise<boolean> => {
    if (styles.length === 0) return false;
    try {
      // Resolves after the catalog, suggestions and orders have been refetched.
